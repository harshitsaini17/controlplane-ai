...
| Per-detector latency vs budget | `python -m eval.bench_latency` | `reports/latency_report.md` | **Five detectors measured; three clearly inside budget, and the verdict on the other two is contested by an OPEN deviation.** `tier1_pii` P99 **0.124 ms**/2 ms · `tier1_blocklist` **0.024 ms**/2 ms · `numeric_claims` **0.309 ms**/5 ms, all with **0 faults**. `tier2_injection` P99 **25.569 ms**/25 ms (n=300, **0 faults**) and `tier2_toxicity` P99 **25.114 ms**/25 ms (n=283, 2 fail_open faults) both read **NO** in the report — but `check_nfr_p002` gates wall-clock while **ADR-036 binds NFR-P-002 to attributable time**, and zero recorded faults at a P99 above budget is arithmetically impossible if the gated series were the enforced one. Filed as `[D3-nfr-p002-gate-reads-the-clock-adr-036-rejected]`; **published as measured wall-clock and deliberately called neither met nor breached** until it is ruled. The remaining **5 are not exercised** — untested, not met |
