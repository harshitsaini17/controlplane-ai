...
| FR-GW-006 | P0 | **Startup canary + usage-sanity invariant** (as ruled by **ADR-028**). At boot, one canary request compares the provider's reported prompt-token count against a **repo-local estimate** — never against a provider endpoint, since both sides would then belong to the party being audited. Fails when the reported count falls outside `[est/max_ratio, est*max_ratio]` **and** `\|reported − est\|` exceeds `min_delta_floor` (defaults 2.0 / 50): **measured-class fails boot**, **dev-class warns loudly and continues** (ADR-018). Detects **gross** accounting corruption; explicitly **not** a fine-grained accounting audit. Any fallback engagement increments `cp_fallback_engaged_total` and logs at WARNING — never silently. | Boot refused for a grossly miscounting measured-class provider; dev-class boot emits the warning; a silenced fallback fails the test. |
...
| FR-POL-003 | P0 | EDIT actions implemented: PII redaction (`[REDACTED:<category>]`) and epistemic softening of low-confidence claims (template-based). Redaction applies at **both** stages — an output sentence before release, and a prompt before dispatch (input EDIT, ADR-020). | Redacted output contains no span of the original match; a redacted prompt reaches the upstream with no span of the original match, and the pre-dispatch re-scan finds nothing (04 §4.5). |
...
|---|---|---|---|
| NFR-P-001 | Gateway hot-path overhead (everything except LLM inference), **streaming pipelines** — re-scoped by **ADR-030** to the **user-perceived hold**: the input-lane hold, and each per-sentence hold (boundary arrival → release). Normative in 06 §4 | **Input-lane hold: P50 < 40 ms, P99 < 50 ms — scoped by ADR-030 Amendment 2 to single-window inputs** (≤104 tokens, the ADR-032 window), where the derivation's dominant `tier2_injection` term is measured at 13.01 ms P99; multi-window input holds are published as an **untargeted window-count-bucketed series** and `--check` gates the single-window population only. **Per-sentence hold: P50 < 40 ms, P99 < 100 ms** on demo hardware (Python prototype; compiled-gateway roadmap in ADR-001). Both **derived from the 04 §2 budgets** in ADR-030, not chosen to fit. The per-request sum (`total_attributable_overhead_ms`) and `added_time_to_last_byte_ms` stay **reported in every latency report with no target** — the latter measured by the **benchmark client**, not by the gateway, per **ADR-030 Amendment 1**: its definition is client-observed and the gateway has no such vantage, so it is a 06 §4 quantity and not a `latency_json` key; non-streaming pipelines (UC-3, ADR-014) likewise. **Measured** against the emitted per-hold series (M-20; figures, sample sizes and verdict in `reports/latency_report.md`). The population is **holds, not requests** — a 10-segment response contributes 10 samples. A run carrying no streaming traffic still renders `not measured`, which is the third state proper and not a pass | `eval/bench_latency.py`, method in 06 §4 |
| NFR-P-002 | Per-detector fast-path budgets | Tier-1 < 2 ms · Tier-2 < 25 ms · self-consistency < 60 ms (incl. 2nd-sample amortization strategy per 04 §2.3). **ADR-032 scopes the Tier-2 figure to single-window inputs** — `tier2_injection` windows its input at 104 tokens, where the budget is measured to hold (13.01 ms P99); two windows measure 25.13 ms P99 and breach, so multi-window inputs are scored in **full** and their cost published as an untargeted window-count-bucketed series rather than truncated to fit. The target is not moved: every input that breached it still does. | same |
...
- Policy highlights: PII → BLOCK (not redact — internal policy choice); `privacy.person` → BLOCK (carries demo beat 4b); low-confidence → PASS with logged flag; `borderline_action: pass` (ADR-017 — consistent with the PASS-and-log posture). **ADR-019 is what keeps beat 4b intact here:** an enriched `privacy.person` fires at its *mapped* action, so `borderline_action: pass` cannot reach it. A follows-host rule would have let a borderline-grounded fabrication about a named employee PASS on the one use case whose strictness is spent precisely on personal data; toxicity moderate → PASS; `risk_appetite: medium`; highest budget ceiling of the three ($800/mo); sampling 5%; `fail_mode: fail_closed` for Tier-1 only, `fail_open` elsewhere.
