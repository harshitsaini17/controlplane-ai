...
| M-55 | **`git_stamp` reports any untracked file as a dirty tree, so read literally `reproducibility_verdict` disqualifies every artifact this repo emits.** `git_stamp` computes `"dirty"` as `None if porcelain is None else bool(porcelain)` — ANY `git status --porcelain` output, including untracked files and the **sibling reports each measurement run writes**. So the tree is dirty *by construction* during a measurement run, every artifact stamps "+ uncommitted changes", and `reproducibility_verdict` calls that NOT CITABLE | Resolved: **logged, not fixed.** Blast radius is one consumer — `eval/spike_window_latency.py` is the only harness that calls `reproducibility_verdict` (the others stamp `git_stamp` and render it), so no currently-published verdict is wrong and this is an M-row rather than a deviation. Not fixed because the honest fix is a judgement about *which* dirt matters (tracked-file modifications, surely; a sibling report the run itself just wrote, surely not), and encoding that judgement is a change to what "citable" means — cheap to write, and exactly the kind of definition that should not be quietly widened at deadline by the agent whose artifacts it grades | PROVISIONAL — batch review at phase end |
