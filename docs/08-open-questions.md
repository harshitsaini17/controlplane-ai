...
| M-18 | **`entity_enricher` is unbounded inside a per-sentence hold.** 04 §2.2 budgets it at < 10 ms **per enriched span** and caps the number of spans nowhere, so the composed hold is `60 + 10k`, which crosses ADR-030's 100 ms P99 at **k = 4** | **Logged, nothing invented.** ADR-030's derivation states the crossing point and records that its own per-sentence target is satisfiable *only* under an enrichment bound no doc currently states. 04 §2.2 needs either a span cap or an explicit exclusion of enrichment from the hold | No doc contradicts another and nothing is failing — enrichment is unimplemented, so there is no measurement to breach. It is logged rather than resolved because **the fix would be a cap I invented to make my own new target fit**, which is the §5.4 move exactly; a real cap needs an owner's ruling on spaCy's per-span cost |
| M-19 | **Policy evaluate + action apply are inside a targeted quantity but unbudgeted.** 06 §4 puts them in the hold ("detector wait + policy + action"); 04 §2 budgets detectors only | **Logged with its current measurement.** The whole step measures well under 1 ms today, so ADR-030's targets absorb it and the derivation says so explicitly instead of quietly omitting the term | An unbudgeted term inside a targeted quantity is a gap whether or not it currently matters, but it contradicts nothing: 04 §2 is a *detector* registry and never claimed to cover the policy step. Worth a budget row if the engine ever grows expensive |
| M-20 | **ADR-030's `latency_json` changes are documented but not yet emitted — the rename included.** The audit write path still writes `gateway_overhead_ms`, not `total_attributable_overhead_ms`; and the three new series `input_hold_ms`, `sentence_holds_ms` and `added_time_to_last_byte_ms` exist as intervals in code (`app.py:414`; `unit_ms` at `app.py:698`) but are **accumulated into one figure**, never recorded separately | **Logged, and its consequence published rather than glossed:** NFR-P-001 is `not measured` from ADR-030 until the write path carries them — old target withdrawn, new series absent. 06 §4 now bars `--check` from gating NFR-P-001 on the renamed sum, and the latency report states the third state | Docs preceding code is this repo's design (§2), so a contract ahead of instrumentation is the normal order, not a mismatch. It is logged because **05 §5's vocabulary is enforced at the write path** by `check_latency_keys`, so "documented but not emitted" is a state a reader must be told about rather than infer from a missing panel |
...
04 §2 budgets** in the ADR, and the derivation logged three gaps of its own (**M-18** enrichment is
unbounded per sentence, **M-19** the policy step is unbudgeted inside a targeted quantity, **M-20**
the new series are not emitted yet) rather than smoothing them over. The lane goes parallel **when
