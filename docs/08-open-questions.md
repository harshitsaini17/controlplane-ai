...
| M-71 | **CI had never been green, and the noise hid three real defects.** The `verify` job installs `.[dev]` only, so `spacy`, `onnxruntime`, `sentence_transformers` and `en_core_web_sm` are absent there while the dev `.venv` carries them; `finance_advisor` maps three of those detectors `fail_closed`, so ADR-033 rule 2 correctly refused the boot and every gateway-booting test failed on the light host. Behind that: (a) `tests/test_run_all.py` read `dut.scope` off `DEMOTED`, which holds `SkippedDetector` (`name`/`labels`/`reason`) — `AttributeError` wherever a dependency is missing, and **vacuously passing** on an ml host where `DEMOTED` is empty, so the assertion had never once run; (b) `warm_detector_models` documented "Never raises for an absent dependency" and raised `ModuleNotFoundError`; (c) `faultable()` in `eval/fault_injection.py` treated `LIVE` membership as proof of loadability, but `run_lane` consults the boot manifest and never calls `detect()` on an unloadable detector, so `_Faulty` never fired and the probe read `failures=[]` — a control run mislabelled as faulted, 23/39 on the light host. | **Fixed at source; gated, not hidden.** (a) reads `dut.labels`. (b) a `_warm_one` helper skips anything in `detectors_unloadable` and catches `ImportError` **narrowly** — an absent dependency is ADR-033 state (c) and boots without it, while a present-but-broken dependency still surfaces as a fault; it deliberately does not backfill the manifest. (c) `faultable()` subtracts a `probe_availability`-derived `UNLOADABLE`, and degraded rows render `— none live —` / Exercisable **no** rather than passing falsely. The host-dependent tests take a `requires_ml` marker whose `MISSING_ML` is derived from ADR-033's `REQUIREMENTS` via `find_spec` (never importing), with the skip applied in `tests/conftest.py` so the marker stays a pure *selector*. `ci.yml`'s `ml-closure` job gained `spacy download en_core_web_sm` — the NER model is its own import name in `REQUIREMENTS`, so without it `MISSING_ML` is non-empty there too and the gated tests would run in **no** job — plus a step that runs `-m requires_ml` and fails the build on any skip or on zero passes. Measured: light host 28 of these tests 0 failures with 25 skipped, ml host 29 passed 0 skipped, `fault_injection` 23/39 -> 27/27, all four gates exit 0 on py3.12 and py3.14. | Not a deviation: the docs were right at every point and the code was wrong. ADR-033's boot refusal is the specified behaviour working, not a contradiction to rule on. Four exact-boot-manifest assertions were **gated rather than relaxed** — `==` was not loosened to `in` (AGENTS.md §5.4) — and one of them had only ever passed because the old `unloadable_tier2` fixture erased the host's real requirements, i.e. it asserted the fixture's fiction. The `faultable()` edit is provably a no-op where the stack is present (`UNLOADABLE == {}`). |
