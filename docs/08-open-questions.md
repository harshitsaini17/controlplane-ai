...
⚠ Two related items are **not** MINOR and are NOT resolved here — both carry open deviation reports and both leave the dataset with known coverage gaps recorded in `eval/dataset/REVIEW_NEEDED.md`: `[D4-enriched-label-survival-semantics]` (no in-band person cases) and `[D4-input-stage-pii-edit-unresolvable]` (no input-stage PII cases).
