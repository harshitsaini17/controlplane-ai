...
| `[D1-tier2-budgets-cannot-coexist-with-nfr-p-001]` | **MAJOR** | Phase 5 | **OPEN** — filed 2026-08-27 from the 06 §4 benchmark's forward projection. NFR-P-001 (streaming P99 < 100 ms) and the 04 §2 per-sentence budget set cannot both hold at the **measured** segment distribution once the documented detector set is complete. Arithmetic over declared budgets, **not** an observed breach — the measured P99 clears the target by a wide margin, stated as a derived factor in the report rather than pinned here — so it is deliberately **not** a D3. Fails under both lane readings (sequential sum *and* parallel max), so it does not rest on `run_lane`'s current sequential choice |

**Open: one** — `[D1-tier2-budgets-cannot-coexist-with-nfr-p-001]`, filed 2026-08-27 (see the report at the end of this file). **17 of 18** filed deviations are ruled and closed. The open one is the first filed from a *projection* rather than from a measurement or a doc reading: it reports that two documents cannot both hold in a future state, which is why it is a D1 and not the D3 its subject matter might suggest. The Phase-5 filing
...
budget projection the **twelfth**: 11 closed + 1 open = 12, and the 6 pre-Step-4 closures bring
the total to **18**.
