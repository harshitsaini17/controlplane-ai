...
were ruled, and the **nineteenth** — `[D2-tier2-served-graph-is-unbuildable-on-the-ml-extra]` — closed 2026-08-29 under **ADR-035**, and the **twentieth** (`[D3-bound-case-window-count-undercovers-the-policy-bound]`) closed the same day under **ADR-032 Correction 1**, leaving the **twenty-first** (`[D1-batch-4-justification-falsified-at-the-corrected-bound]`) and the **twenty-second** (`[D1-two-window-budget-breach-not-reproduced-on-the-clean-artifact]`) open — both surfaced by the twentieth's own re-measurement, which falsified written text at two separate rungs of the same table. That is the third and fourth instance in this workstream of a figure contradicted by the measurement it claims to come from, and the reason Correction 1 carries a mechanical re-derivation check rather than a promise to be careful. D2 stays worth reading for *how it was found*. It
...
Step-4-onward filings** — **20 closed + 2 open = 22** — and the 6 pre-Step-4 filings, all
closed, bring the whole table to **26 closed + 2 open = 28**. Both are stated because the
